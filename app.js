(() => {
  ...
  const destination = s => s.mapQuery || ((s.hotel ? s.hotel + ', ' : '') + s.address);
  const stopMap = s => map(destination(s));
  ...
  function renderRoute(){
    ...
    return '<section class="overview-hero"><img class="hero-art" src="./panorama.png" alt="Illustrated Japanese mountain road, castle and coastal landscape" fetchpriority="high"><div><span class="eyebrow">OCTOBER 2026 · POCKET EDITION</span><h1>The road book.</h1><p class="hero-date">13–22 October <span>Honjo → Honjo</span></p></div></section>' +
      ...
      '<section class="alternative-block"><h2>Alternative reservation</h2><p>Ako · 17–18 October. The itinerary above continues to Himeji.</p>'+card(alt)+'</section>';
  }
  ...
  function renderGallery(s){
    ...
    return '<section class="photo-gallery" aria-label="Photographs of '+escape(s.title)+'"><div class="gallery-heading"><h2>In & around '+escape(s.title)+'</h2><div class="gallery-controls"><button type="button" data-gallery-step="-1" aria-label="Previous photograph">‹</button><span class="gallery-count" aria-live="polite">1 / '+s.photos.length+'</span><button type="button" data-gallery-step="1" aria-label="Next photograph">›</button></div></div><div class="gallery-track" tabindex="0" aria-label="Swipe or use arrow keys to browse photographs">'+s.photos.map((p,i)=>'<figure class="gallery-slide"><img src="'+escape(p.src)+'" alt="'+escape(p.alt)+'" width="'+p.width+'" height="'+p.height+'" loading="'+(i?'lazy':'eager')+'" decoding="async"'+(p.contain?' class="contain"':'')+'><figcaption><span class="photo-caption">'+escape(p.caption)+'</span><details class="photo-credit"><summary>Photo credit</summary><p>'+escape(p.author)+' · <a href="'+escape(p.licenseUrl)+'" target="_blank" rel="noopener noreferrer">'+escape(p.license)+'</a> · <a href="'+escape(p.sourceUrl)+'" target="_blank" rel="noopener noreferrer">Original photograph</a>'+(p.credit?'<br>'+escape(p.credit):'')+(p.creditUrl?' <a href="'+escape(p.creditUrl)+'" target="_blank" rel="noopener noreferrer">Attribution source</a>':'')+'<br>'+escape(p.changes||'Resized for this guide.')+'</p></details></figcaption></figure>').join('')+'</div></section>';
  }
  function renderFlavours(s){
    const f=s.flavours;if(!f)return '';
    return fold('Eat & take home','<p>'+escape(f.lead)+'</p><div class="flavour-list">'+f.items.map(item=>'<section class="flavour-item"><span class="local-kind">'+escape(item.kind)+'</span><h3>'+escape(item.name)+'</h3><p class="local-japanese" lang="ja">'+escape(item.jp)+'</p><p>'+escape(item.text)+'</p><a class="local-find" href="'+escape(map(item.query))+'" target="_blank" rel="noopener noreferrer">Find in Maps ↗</a></section>').join('')+'</div><p class="local-sources">Local guides: '+f.sources.map(source=>'<a href="'+escape(source.url)+'" target="_blank" rel="noopener noreferrer">'+escape(source.label)+'</a>').join(' · ')+'</p>','food-fold');
  }
  function renderYokai(s){
    const entries=(s.yokai||[]).map(id=>guide.yokai[id]).filter(Boolean);
    if(!entries.length)return '';
    return '<section class="identification"><h2 class="section-label">Visual identification</h2><div class="yokai-grid">'+entries.map(y=>{
      const [x,top,w,h]=y.crop;
      const crop='width:'+184*w/h+'px;height:184px;';
      const art='width:'+1536/w*100+'%;height:'+1024/h*100+'%;left:'+(-x/w*100)+'%;top:'+(-top/h*100)+'%;';
      return '<article class="yokai-card"><div class="yokai-image"><span class="yokai-crop" style="'+crop+'"><img src="./assets/yokai-'+y.atlas+'.png" style="'+art+'" alt="'+escape(y.name+': '+y.identify)+'" loading="lazy" decoding="async"></span></div><div class="yokai-description"><span lang="ja" class="yokai-japanese">'+escape(y.jp)+'</span><h3>'+escape(y.name)+'</h3><p>'+escape(y.identify)+'</p><details><summary>Field procedure</summary><p>'+escape(y.protocol)+'</p></details></div></article>';
    }).join('')+'</div></section>';
  }
  function renderExplore(s){
    ...
    return renderGallery(s)+(intro?'<div class="prose">'+intro[0]+'</div>':'') + renderFlavours(s) + pickBlocks +
      fold('The area & its history',history) + fold('A manageable visit',sect['A manageable visit']||'')+
      (s.flavours?.walk?'<div class="arrival-note"><span class="eyebrow">ON FOOT</span><p>'+escape(s.flavours.walk)+'</p></div>':'');
  }
  ...
  function drivingLink(s){
    const originByStop={
      ...
      sekigahara:destination(getStop('ena')),
      maizuru:getStop('sekigahara').parkingQuery||destination(getStop('sekigahara')),
    ...
    let url='https://www.google.com/maps/dir/?api=1&travelmode=driving&origin='+encodeURIComponent(originByStop[s.id])+'&destination='+encodeURIComponent(s.parkingQuery||destination(s));
  ...
  function renderTravel(s){
    ...
    if(s.id==='ena') extra='<div class="notice">The inland journey includes Route 142 between Saku and Okaya. Allow a proper break before the mountain crossing.</div>';
    if(s.id==='sekigahara') extra='<div class="notice">Drive to the memorial museum visitor parking, then explore the battlefield on foot. The separate battlefield link marks the decisive-battle site.</div>'+link(map(s.parkingQuery),s.parkingLabel,'maps-button secondary');
    ...
    return '<section class="route-box"><span class="eyebrow">'+(s.hotel?'YOUR HOTEL':'VISITOR DESTINATION')+'</span><h3>'+escape(s.hotel||s.mapName||s.title)+'</h3><p class="address">'+escape(s.address)+'</p><button type="button" class="copy-address" data-copy="'+escape(s.address)+'">Copy address</button>'+link(stopMap(s),'Open in Google Maps')+(s.mapSource?'<p class="map-source"><a href="'+escape(s.mapSource)+'" target="_blank" rel="noopener noreferrer">Official access information</a></p>':'')+'</section>'+
  ...
  function route(){
    ...
    if(requested==='stop'&&getStop(parts[1])){
      ...
      document.querySelector('meta[name="theme-color"]').content=document.documentElement.dataset.theme==='dark'?'#0b1421':s.color;
    }else{
      ...
      document.querySelector('meta[name="theme-color"]').content=document.documentElement.dataset.theme==='dark'?'#0b1421':'#142841';
    }
    if(renderingStarted){
  ...
  function bindGallery(){
    const gallery=main.querySelector('.photo-gallery');if(!gallery)return;
    const track=gallery.querySelector('.gallery-track');
    const slides=[...track.children],count=gallery.querySelector('.gallery-count');
    const buttons=gallery.querySelectorAll('[data-gallery-step]');
    const index=()=>slides.reduce((best,slide,i)=>Math.abs(slide.offsetLeft-slides[0].offsetLeft-track.scrollLeft)<Math.abs(slides[best].offsetLeft-slides[0].offsetLeft-track.scrollLeft)?i:best,0);
    const update=()=>{const n=index();count.textContent=(n+1)+' / '+slides.length;buttons[0].disabled=n===0;buttons[1].disabled=n===slides.length-1;};
    const advance=step=>{const n=Math.max(0,Math.min(slides.length-1,index()+step));track.scrollTo({left:slides[n].offsetLeft-slides[0].offsetLeft,behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});};
    gallery.addEventListener('click',event=>{const b=event.target.closest('[data-gallery-step]');if(b)advance(Number(b.dataset.galleryStep));});
    track.addEventListener('keydown',event=>{if(event.key==='ArrowRight'||event.key==='ArrowLeft'){event.preventDefault();advance(event.key==='ArrowRight'?1:-1);}});
  ...
  function updateOfflineText(){
    ...
    document.getElementById('cache-message').textContent=savedOffline?'Every chapter, photograph and identification card is saved on this device for offline reading.':workerRegistration?'Saving the guide and its pictures for offline reading. Keep this page open until it confirms it is saved.':'Open the hosted guide while online to prepare the offline copy.';
  }
